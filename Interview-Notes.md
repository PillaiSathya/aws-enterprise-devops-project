...
EC2 User Data is a script or configuration provided during instance launch that can be executed during the instance's initial bootstrapping. It is commonly used to automate initial server configuration, such as installing packages, starting services, and deploying application files.

Q: What problem did you face with User Data?

I initially changed the Terraform User Data, but the existing EC2 instance did not rerun the initialization script. I resolved this by configuring user_data_replace_on_change = true, which causes Terraform to replace the instance when User Data changes. I then verified that the new instance automatically installed Nginx and deployed the custom HTML page.

Q: How did you verify the deployment?

I first obtained the EC2 public IP through Terraform output and then tested the application using both a browser and curl. The curl response confirmed that Nginx was serving the custom HTML generated by the User Data script.
...
Terraform variables allow us to parameterize infrastructure configuration instead of hardcoding environment-specific values.
...
This makes the Terraform configuration reusable and easier to maintain.

Why did I use t3.micro?

I used t3.micro because this project is being developed within AWS Free Tier/available credits and I wanted to control infrastructure costs.

I parameterized the instance type using a Terraform variable while keeping the default value as t3.micro.

Why did I parameterize tags?
...
This allows the same Terraform configuration to be reused for different environments.

What was the result after introducing variables?
I ran:
...
This confirmed that changing hardcoded values to variables did not change the existing AWS infrastructure.